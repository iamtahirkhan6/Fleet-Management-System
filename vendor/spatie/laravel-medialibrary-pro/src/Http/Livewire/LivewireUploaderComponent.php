...
class LivewireUploaderComponent extends Component
{
    ...
    public function mount(string $rules, bool $multiple = false, string $uuid = null, bool $add = false)
    {
        $this->rules = $rules;
    ...
    public function updatedUpload()
    {
        ...
        if (! is_null($uploadError)) {
            ...
            if (! $this->add) {
                $this->emit('uploadError', $this->uuid, $uploadError);
            }
    ...
    protected function getUploadError(): ?string
    {
        ...
        try {
            $this->validate([
                $field => $this->rules,
            ], ["{$field}.mimes" => 'You must upload a file of type :values']);
    ...
    public function uploadErrored($name, $errorsInJson, $isMultiple)
    {
        try {
            ...
            $this->add
                ? $this->emitUp('showListErrorMessage', $uploadError)
                : $this->emit('uploadError', $this->uuid, $exception->validator->errors()->first());
        }
    }
