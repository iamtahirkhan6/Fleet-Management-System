...
async function vaporUpload(file, options = { signedStorageUrl: '/vapor/signed-storage-url', withCredentials: true }) {
    const response = await axios$1.post(options.signedStorageUrl, {
    ...
    await axios$1.put(response.data.url, file, {
        withCredentials: options.withCredentials,
...
function getCancelTokenSource() {
    return axios$1.CancelToken.source();
}
async function uploadFile({ routePrefix, file, uuid, cancelTokenSource, vapor, vaporSignedStorageUrl, uploadDomain, withCredentials, onUploadProgress, }) {
    if (vapor) {
        ...
        return axios$1.post(uploadDomain + '/' + routePrefix + '/post-s3', {
            uuid: response.uuid,
            key: response.key,
    ...
    return axios$1.post(uploadDomain + '/' + routePrefix + '/uploads', formData, {
        withCredentials,
...
class MediaLibraryClass {
    ...
    async getFileValidationErrors(file) {
        ...
        try {
            await Promise.resolve(this.config.beforeUpload(file));
        }
        catch (error) {
            if (error) {
...
function cleanRule(rule) {
    ...
    if (lowerCaseRule.startsWith('image/') || lowerCaseRule.startsWith('application/')) {
...
function buildRuleHelpText({ validationRules = {}, maxItems, }) {
