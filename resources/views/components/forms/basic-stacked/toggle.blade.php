<button @click="{{ $attributes["alpineBool"] }} = !{{ $attributes["alpineBool"] }}" :class="{ 'bg-indigo-600' : {{ $attributes["alpineBool"] }}, 'bg-gray-200' : !{{ $attributes["alpineBool"] }}}" type="button"
        aria-pressed="false"
        class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
    <span class="sr-only">{{ $slot }}</span>
    <span :class="{ 'translate-x-5' : {{ $attributes["alpineBool"] }}, 'translate-x-0' : !{{ $attributes["alpineBool"] }}}"
    ...
    <span :class="{ 'opacity-0 ease-out duration-100' : {{ $attributes["alpineBool"] }}, 'opacity-100 ease-in duration-200' : !{{ $attributes["alpineBool"] }}}"
    ...
    <span :class="{ 'opacity-100 ease-in duration-200' : {{ $attributes["alpineBool"] }}, 'opacity-0 ease-out duration-100' : !{{ $attributes["alpineBool"] }}}"
