        ...
        <ol class="border border-gray-300 divide-y divide-gray-300 rounded-md md:flex md:divide-y-0">
            ...
            <li class="relative md:flex-1 md:flex">
                <a href="#" class="flex items-center group">
                    <!-- When Step 2 is inactive --><span x-show="!stepOneCompleted" class="flex items-center px-6 py-4 text-sm font-medium">
                        <span
                            ...
                            Documents</span> </span>
                    <!-- When Step 2 is active --><span class="flex items-center px-6 py-4 text-sm font-medium" x-show="step2" x-cloak>
                        <span
                            ...
                            Documents</span> </span>
                    <!-- When Step 2 is completed --><span class="flex items-center px-6 py-4 text-sm font-medium" x-show="stepTwoCompleted" x-cloak>
                        <span
        ...
        <form class="min-h-full space-y-4" wire:submit.prevent="createTrip()">
            <div class="px-4 py-5 sm:p-6">
                ...
                <div x-show="step1" x-cloak>
                    ...
                    <div class="grid grid-cols-1 gap-y-6 gap-x-4 @if($mActive != 'true') sm:grid-cols-2 @else
                        ...
                        mentioned on the challan." class="w-11/12 col-span-1 border-gray-200 sm:border-r">
                            <!-- Loading Point -->
                            <x-forms.below-stacked.row-with-value title="Source"> {{ $source }} </x-forms.below-stacked.row-with-value>
                            <!-- Unloading Point -->
                            <x-forms.below-stacked.row-with-value title="Destination"> {{ $destination }} </x-forms.below-stacked.row-with-value>
                            <!-- Consignee -->
                            <x-forms.below-stacked.row-with-value title="Consignee"> {{ $consignee }} </x-forms.below-stacked.row-with-value>
                            <!-- Date -->
                            <x-forms.below-stacked.row title="Date" error="input.date">
                                <x-date-picker wire:model.debounce.500ms="input.date" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" /> </x-forms.below-stacked.row>
                            <!-- Challan Serial -->
                            <x-forms.below-stacked.row title="Challan Serial" error="input.challan_serial">
                                <x-forms.below-stacked.text-input type="number" autocomplete="off" wire:model.debounce.500ms="input.challan_serial" /> </x-forms.below-stacked.row>
                            <!-- Vehicle Number -->@if ($mActive == 'true')
                            <!-- Market Vehicle -->
                                <x-forms.below-stacked.row title="Vehicle Registration Number" error="input.market_vehicle_number" required="true">
                                    <x-forms.below-stacked.text-input type="text" wire:model.debounce.500ms="input.market_vehicle_number" /> </x-forms.below-stacked.row> @else
                            <!-- Fleet Vehicle -->
                                <x-forms.below-stacked.row title="Vehicle Registration Number" error="input.fleet_vehicle_id" required="true">
                                    <x-forms.below-stacked.dropdown title="Select a Vehicle" :array="$fleet_vehicles" arrayKey="number" wire:model.debounce.500ms="input.fleet_vehicle_id"> </x-forms.below-stacked.dropdown>
                                </x-forms.below-stacked.row> @endif
                        <!-- TP Number -->
                            <x-forms.below-stacked.row title="TP Number" error="input.tp_number" required="true">
                                <x-forms.below-stacked.text-input type="text" wire:model.debounce.500ms="input.tp_number" /> </x-forms.below-stacked.row>
                            <!-- TP Serial -->
                            <x-forms.below-stacked.row title="TP Serial" error="input.tp_serial" required="true">
                                <x-forms.below-stacked.text-input type="number" wire:model.debounce.500ms="input.tp_serial" /> </x-forms.below-stacked.row> @if ($mActive == 'true' && $step0 == false)
                            <!-- Agent -->
                                <x-forms.below-stacked.row title="Agent" error="input.agent_id">
                                    <x-forms.below-stacked.dropdown :array="$agents" arrayKey="name" title="Select an
                                     agent" wire:model.debounce.500ms="input.agent_id" /> </x-forms.below-stacked.row> @endif </x-forms.below-stacked.vertical-column>
                        ...
                        <x-forms.below-stacked.vertical-column title="Quantity Information" desc="Fill the details as mentioned on the challan." class="w-11/12 col-span-1 border-gray-200 {{ ($oActive != 'true') ? 'sm:border-r':'' }}">
                            ...
                            <x-forms.below-stacked.row title="Gross Weight" error="input.gross_weight">
                                <x-forms.below-stacked.text-input type="number" wire:model.debounce.500ms="input.gross_weight" /> </x-forms.below-stacked.row>
                            <!-- Tare Weight -->
                            <x-forms.below-stacked.row title="Tare Weight" error="input.tare_weight">
                                <x-forms.below-stacked.text-input type="number" wire:model.debounce.500ms="input.tare_weight" /> </x-forms.below-stacked.row>
                            <!-- Net Weight -->
                            <x-forms.below-stacked.row title="Net Weight" error="input.net_weight" required="true">
                                <x-forms.below-stacked.text-input type="number" wire:model.debounce.500ms="input.net_weight" /> </x-forms.below-stacked.row>
                            <!-- Rate -->
                            <x-forms.below-stacked.row title="Rate per metric ton" error="input.rate" required="true">
                                <x-forms.below-stacked.text-input type="number" wire:model.debounce.500ms="input.rate" /> </x-forms.below-stacked.row>
                            ...
                            <!-- Driver Name -->
                                <x-forms.below-stacked.row title="Driver Name" error="input.driver_name">
                                    <x-forms.below-stacked.text-input type="text" wire:model.debounce.500ms="input.driver_name" /> </x-forms.below-stacked.row>
                                <!-- Driver Phone Number -->
                                <x-forms.below-stacked.row title="Phone Number" error="">
                                    <x-forms.below-stacked.text-input type="tel" maxlength="10" wire:model.debounce.500ms="input.driver_phone_num" /> </x-forms.below-stacked.row>
                                <!-- Driver License Number -->
                                <x-forms.below-stacked.row title="License Number" error="">
                                    <x-forms.below-stacked.text-input type="text" wire:model.debounce.500ms="input.driver_license_num" /> </x-forms.below-stacked.row> @elseif($oActive == 'true')
                                <x-forms.below-stacked.row title="Driver" error="input.fleet_driver_id" required="true">
                                    <x-forms.below-stacked.dropdown title="Select a driver" :array="$paid_drivers" arrayKey="name" wire:model.debounce.500ms="input.fleet_driver_id" /> </x-forms.below-stacked.row> @endif </x-forms.below-stacked.vertical-column> @if($oActive != 'true')
                        <!-- Cash Advance Information -->
                            <x-forms.below-stacked.vertical-column title="Cash Advance Information" desc="Fill the details as mentioned on the challan." class="col-span-1">
                                ...
                                <x-forms.below-stacked.row title="HSD Amount" error="input.hsd">
                                    <x-forms.below-stacked.text-input type="number" wire:model.debounce.500ms="input.hsd" /> </x-forms.below-stacked.row>
                                <!-- Cash Amount -->
                                <x-forms.below-stacked.row title="Cash Amount" error="input.cash">
                                    <x-forms.below-stacked.text-input type="number" wire:model.debounce.500ms="input.cash" /> </x-forms.below-stacked.row>
                                <!-- Owner Information -->
                                ...
                                <x-forms.below-stacked.row title="Party Name" error="input.party_name">
                                    <x-forms.below-stacked.text-input type="text" wire:model.debounce.500ms="input.party_name" /> </x-forms.below-stacked.row>
                                <!-- Owner Phone Number -->
                                <x-forms.below-stacked.row title="Phone Number" error="input.party_number">
                                    <x-forms.below-stacked.text-input type="tel" maxLength="10" wire:model.debounce.500ms="input.party_number" /> </x-forms.below-stacked.row>
                            </x-forms.below-stacked.vertical-column> @endif
                        @error('*') {{ $message }} @enderror
                    </div>
                </div>
                <!-- Step 2 -->
                <div x-show="step2" class="grid grid-cols-1" x-cloak>
                    <div class="col-span-1 border-gray-200 sm:pr-7">
                        ...
                        <!-- Upload Challan -->
                        <div class="mt-6 w-full md:w-2/6">
                            <label class="block text-sm font-medium text-gray-700"> Upload Challan </label>
                            <x-media-library-attachment name="challan_soft_copy" rules="mimes:jpeg,png"/>
                            @error('challan_soft_copy')<p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="step3" class="grid-cols-1" x-cloak>
                    <x-description-list.main title="Review Trip Information" desc="Re-check every details before submitting the form.">
                        ...
                        <x-description-list.row :value="$source ?? null" background="bg-white">Source</x-description-list.row>
                        <x-description-list.row :value="$destination ?? null" background="bg-gray-50">Destination</x-description-list.row>
            ...
            <div class="px-4 py-4 bg-gray-800 sm:px-6">
                <div class="flex justify-end">
                    ...
                    <button x-show="step3" type="submit" class="inline-flex justify-center px-4 py-2 ml-3 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" x-cloak> Submit </button>
                </div>
            </div>
        </form>
        <!-- Success Modal -->
