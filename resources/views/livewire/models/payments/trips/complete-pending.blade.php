<div class="overflow-hidden bg-white border-t-4 border-indigo-400 rounded-lg shadow-lg"
    ...
    <x-forms.basic-stacked.form wire:submit.prevent='completePayment' title="Party Details"
        ...
        <x-forms.basic-stacked.column title="PAN" error="input.pan" width="md:w-2/6 sm:w-full" x-show="panDiv">
            ...
            <button wire:click="checkIfPartyExists" type="button" class="mt-4 inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Search Party!
            </button>
        ...
        <div x-show="personalDetails" x-cloak>
            <x-forms.basic-stacked.column title="Name on Pan" error="input.name" x-cloak>
                <x-forms.basic-stacked.input-basic wire:model.lazy="input.name" type="text" placeholder="Enter the name"></x-forms.basic-stacked.input-basic>
            </x-forms.basic-stacked.column>
            <x-forms.basic-stacked.column title="PAN" error="input.pan" x-cloak>
                <x-forms.basic-stacked.input-basic wire:model.lazy="input.pan" type="text" placeholder="Enter the pan" maxlength="10" oninput="let p=this.selectionStart;this.value=this.value.toUpperCase();this.setSelectionRange(p, p);"></x-forms.basic-stacked.input-basic>
            </x-forms.basic-stacked.column>
            <x-forms.basic-stacked.column title="Phone Number" error="input.phone_number" x-cloak>
                <x-forms.basic-stacked.input-basic wire:model.lazy="input.phone_number" type="tel" placeholder="Enter the phone number" maxlength="10"></x-forms.basic-stacked.input-basic>
            </x-forms.basic-stacked.column>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 gap-y-3 md:grid-cols-2 md:gap-x-6"
            ...
            @forelse($existing_bank_accounts as $bank_account)
                    <div class="grid-cols-1 bg-white overflow-hidden shadow-md rounded-lg divide-y divide-gray-200 border-t-2 border-yellow-300 mt-2">
        ...
        <!-- If a bank exists -->
        <div>
            <x-forms.basic-stacked.column title="Select an Account" error="input.bank_account_id" x-show="showExistingBankDetails" x-cloak>
                <x-forms.basic-stacked.dropdown :array="$existing_bank_accounts" arrayKey="account_number"
                                                title="Select an option" wire:model="input.bank_account_id"></x-forms.basic-stacked.dropdown>
            </x-forms.basic-stacked.column>
        ...
        <div class="space-y-8 sm:space-y-5" x-show="showFeesStructure" x-cloak>
            ...
            <!-- Cash Percentage -->
                <x-forms.basic-stacked.column title="Cash Adv. Percentage">
                    <x-forms.basic-stacked.input-basic wire:model="input.cash_adv_pct" type="number" placeholder="%" class="sm:w-1/6" value="0"></x-forms.basic-stacked.input-basic>
                </x-forms.basic-stacked.column>
            ...
            <div class="space-y-8 sm:space-y-5">
                ...
                <x-forms.basic-stacked.column title="TDS Category" error="input.tax_category_id" x-show="!tds_sbm_bool">
                    <x-forms.basic-stacked.dropdown :array="$tax_categories" arrayKey="section" wire:model="input.tax_category_id" title="Select a category"></x-forms.basic-stacked.dropdown>
                </x-forms.basic-stacked.column>
                ...
                <x-forms.basic-stacked.column title="TDS Soft Copy" error="tds_soft_copy" width="md:w-3/6 sm:w-full" x-show="tds_sbm_bool" x-cloak>
            ...
            <x-forms.basic-stacked.column title="2PAY Charges" error="input.two_pay">
                <x-forms.basic-stacked.input-basic wire:model.lazy="input.two_pay" type="number" placeholder="" value="0"></x-forms.basic-stacked.input-basic>
            </x-forms.basic-stacked.column>
        ...
        <div x-show="onPanSearch" x-cloak>
            <x-forms.basic-stacked.column title="Payment Method" error="input.payment_method_id">
                <x-forms.basic-stacked.dropdown :array="$payment_methods" wire:model="input.payment_method_id"></x-forms.basic-stacked.dropdown>
            </x-forms.basic-stacked.column>

            <x-forms.basic-stacked.column title="Payment Status" error="input.payment_status_id">
                <x-forms.basic-stacked.dropdown :array="$payment_statuses" wire:model="input.payment_status_id"></x-forms.basic-stacked.dropdown>
            </x-forms.basic-stacked.column>
        </div>
        ...
        <!-- RC Soft Copy -->
        <x-forms.basic-stacked.column title="Vehicle Registration Soft Copy" width="md:w-3/6 sm:w-full" error="rc_soft_copy"
