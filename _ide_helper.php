 ...
 */
    ...
    namespace Illuminate\Support\Facades {use PDO;use Closure;use stdClass;use Countable;use Exception;use Throwable;use Generator;use SplFileInfo;use DateInterval;use PDOStatement;use LogicException;use Swift_Transport;use App\Models\User;use RuntimeException;use DateTimeInterface;use App\Console\Kernel;use ReflectionException;use Illuminate\Bus\Batch;use BadMethodCallException;use Illuminate\Mail\Mailer;use Illuminate\View\Factory;use SessionHandlerInterface;use Psr\Log\LoggerInterface;use InvalidArgumentException;use Illuminate\Session\Store;use Illuminate\Routing\Router;use Illuminate\Log\LogManager;use Illuminate\Queue\SyncQueue;use Illuminate\Cache\FileStore;use Illuminate\Auth\TokenGuard;use Illuminate\Container\array;use Doctrine\DBAL\DBALException;use Illuminate\Mail\PendingMail;use Illuminate\Mail\MailManager;use Illuminate\Database\Grammar;use Doctrine\DBAL\Schema\Column;use Illuminate\Cookie\CookieJar;use Illuminate\Bus\PendingBatch;use Illuminate\Auth\AuthManager;use Illuminate\Http\UploadedFile;use Illuminate\Http\JsonResponse;use Illuminate\Config\Repository;use Illuminate\Cache\TaggedCache;use Illuminate\Auth\SessionGuard;use Illuminate\Routing\Redirector;use Illuminate\Support\Collection;use Illuminate\Queue\QueueManager;use Illuminate\Cache\CacheManager;use Illuminate\Database\Connection;use Illuminate\Contracts\Queue\Job;use Illuminate\Hashing\HashManager;use Illuminate\Hashing\ArgonHasher;use Illuminate\Console\Application;use Illuminate\Routing\UrlGenerator;use Illuminate\Hashing\BcryptHasher;use Illuminate\Auth\Access\iterable;use Illuminate\Encryption\Encrypter;use Illuminate\Contracts\Cache\Lock;use Illuminate\Contracts\Auth\Guard;use Illuminate\Contracts\View\Engine;use Illuminate\Http\RedirectResponse;use Illuminate\Filesystem\Filesystem;use Illuminate\Session\SessionManager;use Illuminate\Routing\RouteRegistrar;use Illuminate\Hashing\Argon2IdHasher;use Illuminate\Support\LazyCollection;use Illuminate\Database\Query\Builder;use Illuminate\Routing\RouteCollection;use Illuminate\Routing\ResponseFactory;use Illuminate\Contracts\Mail\Mailable;use Illuminate\Support\ServiceProvider;use Illuminate\View\ViewFinderInterface;use GuzzleHttp\Promise\PromiseInterface;use Illuminate\Database\MySqlConnection;use Illuminate\Database\DatabaseManager;use League\Flysystem\FilesystemInterface;use Illuminate\Database\Query\Expression;use Illuminate\Contracts\Support\Htmlable;use League\Flysystem\AwsS3v3\AwsS3Adapter;use Illuminate\Contracts\Filesystem\Cloud;use Illuminate\Http\Client\PendingRequest;use Illuminate\Contracts\Events\Dispatcher;use Illuminate\View\Engines\EngineResolver;use Illuminate\Contracts\Support\Arrayable;use Illuminate\Translation\MessageSelector;use Illuminate\Foundation\Bus\PendingChain;use Illuminate\Contracts\Auth\UserProvider;use Illuminate\Filesystem\FilesystemAdapter;use Illuminate\Filesystem\FilesystemManager;use Illuminate\Database\Schema\MySqlBuilder;use Illuminate\Notifications\ChannelManager;use Illuminate\Contracts\Translation\Loader;use Illuminate\Http\Client\ResponseSequence;use Illuminate\View\Compilers\BladeCompiler;use Illuminate\Auth\AuthenticationException;use Illuminate\Contracts\Auth\StatefulGuard;use Illuminate\Contracts\Container\Container;use Illuminate\Contracts\Auth\PasswordBroker;use Illuminate\Support\Testing\Fakes\BusFake;use Illuminate\Broadcasting\BroadcastManager;use Illuminate\Broadcasting\PendingBroadcast;use Illuminate\Validation\ValidationException;use Illuminate\Support\Testing\Fakes\MailFake;use Illuminate\Contracts\Auth\Authenticatable;use Illuminate\Foundation\Bus\PendingDispatch;use Symfony\Component\Console\Command\Command;use Illuminate\Support\Testing\Fakes\QueueFake;use Illuminate\Contracts\Auth\CanResetPassword;use Illuminate\Support\Testing\Fakes\EventFake;use Doctrine\DBAL\Schema\AbstractSchemaManager;use Illuminate\Contracts\Translation\Translator;use Illuminate\Routing\RouteCollectionInterface;use Illuminate\Database\Schema\MySqlSchemaState;use Illuminate\Contracts\Cookie\QueueingFactory;use Illuminate\Foundation\Console\ClosureCommand;use Symfony\Component\HttpFoundation\ParameterBag;use Illuminate\Auth\Access\AuthorizationException;use Illuminate\Routing\PendingResourceRegistration;use Illuminate\Database\Query\Processors\Processor;use Symfony\Component\Console\Input\InputInterface;use Illuminate\Validation\PresenceVerifierInterface;use Illuminate\Auth\Passwords\PasswordBrokerManager;use Illuminate\Database\DatabaseTransactionsManager;use Illuminate\Contracts\Encryption\DecryptException;use Illuminate\Contracts\Encryption\EncryptException;use Illuminate\Broadcasting\Broadcasters\Broadcaster;use Symfony\Component\Console\Output\OutputInterface;use Symfony\Component\HttpFoundation\StreamedResponse;use Illuminate\Support\Testing\Fakes\NotificationFake;use Illuminate\Auth\Passwords\TokenRepositoryInterface;use Illuminate\Contracts\Filesystem\FileExistsException;use Illuminate\Database\Eloquent\ModelNotFoundException;use Symfony\Component\HttpFoundation\BinaryFileResponse;use Illuminate\Routing\Exceptions\UrlGenerationException;use Symfony\Component\HttpKernel\Exception\HttpException;use Illuminate\Contracts\Filesystem\FileNotFoundException;use Illuminate\Contracts\Container\ContextualBindingBuilder;use Illuminate\Contracts\Container\BindingResolutionException;use Symfony\Component\Routing\Exception\RouteNotFoundException;use Symfony\Component\Console\Exception\CommandNotFoundException;use Symfony\Component\HttpKernel\Exception\NotFoundHttpException;
            /**
     *
     *
     ...
     */
        ...
        {
                        return Router::mediaLibrary($baseUrl);
        }
...
')
    ...
    namespace Barryvdh\Debugbar {use Closure;use Exception;use DebugBar\array;use ErrorException;use DebugBar\StorageInterface;use DebugBar\HttpDriverInterface;use DebugBar\DataCollectorInterface;use DebugBar\RequestIdGeneratorInterface;use Symfony\Component\HttpFoundation\Request;use Symfony\Component\HttpFoundation\Response;
            /**
     *
     *
    ...
    namespace Jenssegers\Agent\Facades {use Jenssegers\Agent\CrawlerDetect;
            /**
     *
     *
     */
    ...
    namespace Lorisleiva\Actions\Facades {use Lorisleiva\Actions\ActionManager;
            /**
     *
     *
     * @see ActionManager
    ...
    namespace Laratrust {use Illuminate\Auth\UserInterface;
            /**
     *
     *
     */
    ...
    namespace Illuminate\Testing {use Illuminate\Http\Response;
            /**
     *
     *
     * @mixin Response
...
namespace  {
            use Barryvdh\Debugbar\Facade;use Laratrust\LaratrustFacade;use Illuminate\Support\Collection;use Illuminate\Database\Query\Builder;use Illuminate\Support\LazyCollection;use Lorisleiva\Actions\Facades\Actions;use Illuminate\Database\Eloquent\Model;use Illuminate\Database\Eloquent\Scope;use Illuminate\Database\Query\Expression;use Illuminate\Contracts\Support\Arrayable;use Illuminate\Contracts\Pagination\Paginator;use Illuminate\Database\Query\Grammars\Grammar;use Illuminate\Database\RecordsNotFoundException;use Illuminate\Database\Query\Processors\Processor;use Illuminate\Database\Eloquent\Relations\MorphTo;use Illuminate\Database\Eloquent\Relations\Relation;use Illuminate\Database\MultipleRecordsFoundException;use Illuminate\Database\Eloquent\ModelNotFoundException;use Illuminate\Contracts\Pagination\LengthAwarePaginator;class App extends \Illuminate\Support\Facades\App {}
            ...
            class Laratrust extends LaratrustFacade {}
