...
use app\Domain\Payment\Actions\Auth;
use App\Domain\Payment\Models\Payment;
use Lorisleiva\Actions\Concerns\AsAction;
use app\Domain\Payment\Actions\exception;
...
class CompleteTripPayment
{
    ...
    public function handle($input, Trip $trip, $company_id) : bool|array
    {
        try {
            ...
            if ($is_razorpay_bool) {
                $party->razorpay_contact_id = CreateContact::run($party->id, $company_id, false);
                $party->save();
            }
            ...
            $bank_account_id = (isset($input['bank_account_id'])) ? $input['bank_account_id'] : $party->bankAccounts()->create([
                ...
                'company_id'      => $company_id,
                'fund_account_id' => (($is_razorpay_bool == true) ? CreateFundAccount::run($input['account_name'], $input['account_number'], $input['ifsc_code'], $party->id, $company_id, $party->razorpay_contact_id) : null),
            ])->id;
